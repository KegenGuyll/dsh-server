#!/usr/bin/env node
/**
 * Idempotent auto-install for the dsh-mobile plugin. Run from the container
 ...
 * then inserts the `mobile` row. The installer therefore does NOT hand-edit the
...
const e = (m) => process.stderr.write(`dsh-mobile: ${m}\n`);
 ...
 * Detect that and rewrite the file to a valid empty sequence. This only fires
 * on the malformed combination — it never rewrites a real, block-sequence
 * profile (e.g. one that carries an MCP client row or dsh-github's rows), which
 * is left untouched. The `mobile` row itself lives in this package's bundle
 * layer (registered through dsh.profile.bundles), not in the user patch.
 */
