#!/usr/bin/env node
/**
 * Idempotent auto-install for the dsh-cost plugin. Run from the container
 ...
 * then inserts the `cost` row. The installer therefore does NOT hand-edit the
...
const PACKAGE = "dsh-cost";
const MARKER = join(PROFILE_DIR, `.${PACKAGE}.installed`);

const e = (m) => process.stderr.write(`dsh-cost: ${m}\n`);
